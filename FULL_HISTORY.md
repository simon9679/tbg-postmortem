...
extraction pipeline as opposition detectors** (a lightweight cosine node-merge does run — the
`SDL_COSINE_MERGE` seen in §5 — but it proposes no opposition edges; see §17 for each one's status).
The wall is about *our use of them here*,
...
(`claude-sonnet-4-6`) — from "Christmas is a fraud" to "I can change", 12 turns (an abridged excerpt — all 12 turns, but rows within turns are trimmed):
...
[ 4] This ghost of Marley is humbug. There's more of gravy than of grave about you.
     supernatural warnings dismissed          57%  NEW
[ 5] Marley — you were always a good friend to me. Why are you fettered?
     supernatural warnings dismissed          76%  ▲19%
...
[12] I'll raise my clerk's salary. I'll be a second father to Tiny Tim. Old Scrooge is dead.
  ...
  Core shift   social responsibility is not my concern  91% ▼ 7%  (weakened)
  Turning pt   message 7 — wealth determines happiness, lost loved one to greed
  Latest move  clerk salary raised today, future self seen as changeable, commits to year-round joy
  Holds onto   goals: commits to year-round joy; relationships: second father to Tiny Tim, lost loved one to greed; values: supernatural warnings dismissed
```

*(Reading the traces — two conventions for the same glyph: in a per-turn line, `▼31%` is the
**step this turn** (a delta, "down by 31pp"); in the `COGNITIVE SNAPSHOT` footer, `91% ▼ 7%` is a
**range** — start value then end value ("from 91% down to 7%"), not a 7-point move.)*
...
`social responsibility is not my concern` is printed at 55% (turn 8) and next as `6% ▼31%` (turn 10),
...
[ 2] But secretly I want to quit everything and start a YouTube channel.
     ...
     [excerpt — 'always finish what I start' also moved on turn 2 (91→~76); that row is not shown]
