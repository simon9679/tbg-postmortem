...
CONFIDENCE_HISTORY_MAX = 5  # points per node
...
class TBGEngine:
    ...
    async def load(self, user_id: str) -> UserTBG:
        ...
        if row["nodes_data"]:
            ...
            # Unpack axis_state packed alongside nodes.
            axis_state_data = nodes_raw.pop("__axis_state__", None)
        ...
        if axis_state_data and isinstance(axis_state_data, dict):
            from user_state import UserAxisState
            tbg.axis_state = UserAxisState(**axis_state_data)
    ...
    async def save(self, tbg: UserTBG):
        ...
        # Pack axis_state into nodes_data — no DB schema change needed.
        if tbg.axis_state.positions:
            nodes_data["__axis_state__"] = tbg.axis_state.model_dump(mode="json")
    ...
    def apply_delta(self, tbg: UserTBG, delta: TBGDelta) -> UserTBG:
        ...
        self._normalize_conflicts(tbg, now, evidence_modified)
    ...
    @staticmethod
    def _update_axis_state(tbg: UserTBG) -> None:
        """
        Recompute UserAxisState from the top-N highest-confidence nodes.

        Algorithm:
          1. Collect candidates (confidence >= 0.3, non-empty axis_projection).
             Require at least 3 candidates to produce a meaningful signal.
          2. Take top-N by confidence (TOP_N_NODES=8).
             Prevents low-signal nodes from compressing the mean to ±0.03.
          3. Compute confidence-weighted mean per axis → positions.
          4. Update peak_positions — max absolute value seen per axis, never resets.
          5. Append (message_count, position) to per-axis trajectory; trim to TRAJECTORY_MAX.
          6. Compute rolling std of last VOLATILITY_WINDOW trajectory points → volatility.
        """
        from user_state import TRAJECTORY_MAX, VOLATILITY_WINDOW, TOP_N_NODES

        candidates = [
            n for n in tbg.nodes.values()
            if n.axis_projection and n.confidence >= 0.3
        ]
        if len(candidates) < 3:
            return

        # Top-N by confidence — prevents low-signal nodes from compressing the mean
        contributing = sorted(candidates, key=lambda n: n.confidence, reverse=True)[:TOP_N_NODES]

        axis_names = list(contributing[0].axis_projection.keys())
        total_weight = sum(n.confidence for n in contributing)

        # --- positions: confidence-weighted mean over top-N nodes ---
        new_positions: Dict[str, float] = {}
        for ax in axis_names:
            if total_weight > 0:
                weighted = sum(n.axis_projection.get(ax, 0.0) * n.confidence for n in contributing)
                new_positions[ax] = round(weighted / total_weight, 3)
            else:
                new_positions[ax] = 0.0

        tbg.axis_state.positions = new_positions
        tbg.axis_state.updated_at_message = tbg.message_count

        # --- peak_positions: max absolute value per axis, never resets ---
        for ax, val in new_positions.items():
            prev_peak = tbg.axis_state.peak_positions.get(ax, 0.0)
            if abs(val) > abs(prev_peak):
                tbg.axis_state.peak_positions[ax] = val

        # --- trajectory: append current positions, cap at TRAJECTORY_MAX ---
        for ax, pos in new_positions.items():
            traj = tbg.axis_state.trajectory.setdefault(ax, [])
            traj.append([float(tbg.message_count), pos])
            if len(traj) > TRAJECTORY_MAX:
                traj[:] = traj[-TRAJECTORY_MAX:]

        # --- volatility: rolling std over last VOLATILITY_WINDOW points ---
        for ax in axis_names:
            traj = tbg.axis_state.trajectory.get(ax, [])
            window = [pt[1] for pt in traj[-VOLATILITY_WINDOW:]]
            if len(window) >= 2:
                tbg.axis_state.volatility[ax] = round(statistics.stdev(window), 3)
            else:
                tbg.axis_state.volatility[ax] = 0.0
    ...
    def _update_node(self, tbg: UserTBG, node: BeliefNode, now: datetime) -> Optional[str]:
        ...
        if existing:
            ...
            # Axis projection — computed once on creation (label is immutable).
            # Wrapped in try/except: projection failure must never block belief update.
            if not node.axis_projection:
                try:
                    node.axis_projection = get_belief_axes().project(node.label)
                except Exception as _e:
                    logger.debug(f"axis_projection failed for '{node.label}': {_e}")
            return node.id
    ...
    def _prune(self, tbg: UserTBG, now: datetime):
        ...
        if len(filtered_nodes) > MAX_NODES:
            sorted_nodes = sorted(
                ...
                key=lambda x: (x[1].source == "explicit", x[1].confidence),
                reverse=True
            )
...
async def update_tbg_background(
    ...
    try:
        ...
        import os
        _USE_DET = os.getenv("TBG_DETERMINISTIC_EXTRACTOR", "0") == "1"

        if _USE_DET:
            from tbg_extractor import _det_extractor
            loop = asyncio.get_running_loop()
            delta = await loop.run_in_executor(
                None, lambda: _det_extractor.extract(user_text, tbg)
            )
        else:
