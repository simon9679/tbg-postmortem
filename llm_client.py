...
Provider is selected via env var LLM_PROVIDER:
  LLM_PROVIDER=gemini    -> Gemini API (nominal default — see NOTE)
  LLM_PROVIDER=openai    -> OpenAI API
  LLM_PROVIDER=anthropic -> Anthropic API
  LLM_PROVIDER=groq      -> Groq API (OpenAI-compatible)
  LLM_PROVIDER=cerebras  -> Cerebras API (OpenAI-compatible). Free tier: 5 req/min
                           + 30K tok/min + 1M tok/day — the req/min bucket is the real wall.

Default configs (per-provider fallbacks when LLM_MODEL/MAX_TOKENS are unset):
  ...
  Groq:      llama-3.3-70b-versatile, t=0, max_tokens=1024 (3000 for reasoning models: gpt-oss/glm)
  Cerebras:  zai-glm-4.7, t=0, max_tokens=5000 (config default; in practice always
             overridden via LLM_MODEL to gpt-oss-120b — the workhorse for repeated runs)

Override via env: LLM_MODEL, LLM_TEMPERATURE, LLM_MAX_TOKENS.

NOTE: the 'gemini' default is legacy (its key is blocked); always set LLM_PROVIDER
explicitly (cerebras in practice). Relying on the default selects a dead provider.
"""
...
async def _cerebras_call(prompt: str, *, timeout: float) -> str:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        for attempt in range(4):
            ...
            # Diagnostic (opt-in): log which rate-limit bucket is empty (day vs
            # minute) instead of guessing. Guarded -> byte-identical when off.
            if os.getenv("CEREBRAS_LOG_HEADERS") == "1":
                try:
                    _rl = {k: v for k, v in response.headers.items() if "ratelimit" in k.lower()}
                    with open("cerebras_ratelimit.log", "a", encoding="utf-8") as _f:
                        _f.write(f"{response.status_code} {_rl}\n")
                except Exception:
                    pass
