...
class BeliefNode(BaseModel):
    ...
    # Legacy field — no longer populated. Retained for backward-compatible
    # deserialization of older graphs. None on all nodes created by the LLM path.
    ...
    # Legacy field — no longer populated (EPA projection removed). Retained for
    # backward-compatible deserialization. Empty dict on all current nodes.
    ...
    # Wheel-of-life domain — set by the extractor from raw facts.
    # career|money|health|relationships|family|lifestyle|meaning|self|other | "" (unknown)
    # Stored for diagnostics; independent from `category` (8-way legacy taxonomy).
...
class UserTBG(BaseModel):
    ...
    def summary(self) -> str:
        ...
        # Conflicts — rank-based, no absolute floor. verbalized edge confidence is
        # uncalibrated (systematic over/under-confidence, saturation); absolute
        # thresholds unfounded — rank-based display, confidence shown ordinally.
        conflicts = sorted(
            (e for e in self.edges.values()
             if e.relation in ("contradicts", "conflicts_with", "blocks")),
            key=lambda e: e.confidence, reverse=True,
        )[:6]
        conflicts = [e for e in conflicts
                     if e.source_id in self.nodes and e.target_id in self.nodes]
        if conflicts:
            parts.append("conflicts: " + ", ".join(
                f"{self.nodes[e.source_id].label}<-X->{self.nodes[e.target_id].label}"
                f"({e.confidence:.0%})" for e in conflicts))
